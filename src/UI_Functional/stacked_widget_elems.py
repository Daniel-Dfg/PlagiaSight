"""
This file defines the UI elements of each window
the user will go through when using the application.

The logic in the classes provided here is only related to UI components themselves,
not to the actual handling of the files provided by the user and the resulting
text analysis.

RELATED FILES :
    - Text analysis itself, from a given set of files : /src/text_analysis.py
    - UI components linked to those in this file, as well as classes that handle
        files storing : ./utilities.py
"""
...
from text_analysis import UnprocessableTextContent, FreqDist
...
GRAPH_COMMON_CHARS_SIMPLE = ["word frequency (top ???%)", "Sentences length (plotting)"] #TODO : edit these names to be more explicit
...
class Step0_WelcomingMessage(QWidget):
    def __init__(self, main_window):
        ...
        buttons = {button1: (lambda: self._proceed_to_next_step(1)),
                   button2: (lambda: self._proceed_to_next_step(MAX_FILES_AMOUNT))}
    ...
    def _proceed_to_next_step(self, max_files_amount):
...
class Step1_FileDropAndCheck(QWidget):
    def __init__(self, main_window):
        ...
        self.main_window = main_window
        layout = QVBoxLayout()
        self.introductory_label = QLabel("Drop your files or directories below or click 'Browse':")
        layout.addWidget(self.introductory_label)
        ...
        if self.main_window.max_files_amount == MAX_FILES_AMOUNT:
            ...
            self.browse_dirs_button.clicked.connect(self._browse_directory)
            layout.addWidget(self.browse_dirs_button)

        self.browse_files_button = QPushButton("Browse files")
        self.browse_files_button.clicked.connect(self._browse_single_file)
        ...
        back_and_next_buttons = QHBoxLayout()
        self.back_button = QPushButton("Back")
        self.back_button.clicked.connect(self._go_back)
        back_and_next_buttons.addWidget(self.back_button)

        self.next_button = QPushButton("Next")
        self.next_button.setEnabled(False)
        self.next_button.clicked.connect(self.proceed_to_next_step)
        back_and_next_buttons.addWidget(self.next_button)

        layout.addLayout(back_and_next_buttons)
        self.setLayout(layout)

    def _update_ui_after_content_drop(self):
    ...
    def _update_current_content_validity(self):
        """
        Updates status, warnings, and (un)allows the user to
        proceed to the next step accordingly.
        """
        ...
        if num_valid_files >= self.drop_area.min_file_amount:
            ...
            self._show_warning(f"Please drop at least {self.drop_area.min_file_amount} valid file{'s' if self.drop_area.min_file_amount != 1 else ''}.")

    def _show_warning(self, message):
        self.warning_label.setText(message)

    def _go_back(self): #to consider : add a generic method for all "go back" buttons ?
    ...
    def _browse_directory(self):
        ...
        if directory_path:
            ...
            self._update_ui_after_content_drop()
            self._update_current_content_validity()

    def _browse_single_file(self): #text will be updated when we'll support more file formats
        ...
        if file_name:
            if self.drop_area.file_format_is_valid(file_name):
                self.drop_area.correct_files.append(file_name) #PROBLEM TODO : access to a public attribute !!
                self._add_file_to_correct_files_list_UI(file_name, file_is_valid=True)
                self._update_current_content_validity()
            else:
                self._show_warning("Please select only .txt files.")
    ...
    def _add_file_to_correct_files_list_UI(self, file, file_is_valid=True):
        ...
        if file_is_valid:
            ...
            remove_button.clicked.connect(lambda: self._remove_file_from_list(file, item, file_is_valid))
    ...
    def _remove_file_from_list(self, file, item, valid=True):
        if valid:
    ...
    def eventFilter(self, watched, event):
        if event.type() == QEvent.Type.Enter:
            remove_file_button = watched.findChild(QPushButton)
            if isinstance(remove_file_button, QPushButton) and remove_file_button:
                remove_file_button.setVisible(True)
        elif event.type() == QEvent.Type.Leave:
            remove_file_button = watched.findChild(QPushButton)
            if isinstance(remove_file_button, QPushButton) and remove_file_button:
                remove_file_button.setVisible(False)
...
class Step2_AnalysisComplexityPick(QWidget):
    def __init__(self, main_window):
        ...
        self.introductory_label = QLabel("Choose the type of analysis you want to perform:")
        layout.addWidget(self.introductory_label)
        ...
        self.launch_button.clicked.connect(self._proceed_to_next_step)
        ...
        self.simple_analysis_radio.toggled.connect(self._update_description)
        self.complex_analysis_radio.toggled.connect(self._update_description)
    ...
    def _update_description(self):
        if self.simple_analysis_radio.isChecked():
            self.description_label.setText("Simple Analysis: A quick overview of the files.") #TODO : change that sample text
    ...
    def _proceed_to_next_step(self):
        analysis_complexity = "simple" if self.simple_analysis_radio.isChecked() else "complex"
        self.main_window.step3_widget = Step3_LoadResults(self.main_window, analysis_complexity)
...
class Step3_LoadResults(QWidget):
    ...
    def __init__(self, main_window, analysis_complexity):
        ...
        self.done.connect(lambda: QTimer.singleShot(200, self._move_to_step4))

        self.setLayout(layout)

        self.comparison_content = None
        QTimer.singleShot(60, self._process_files)
        #self.process_files()
    ...
    def _process_files(self):
        ...
        QTimer.singleShot(50, self.done.emit)


    def _move_to_step4(self):
...
class Step4_DisplayResults(QWidget):
    def __init__(self, main_window):
        ...
        if isinstance(self.main_window.final_results, OneFileComparison):
            ...
            all_correct_files = self.main_window.step1_widget.drop_area.correct_files
            self.left_content_title.addItems(all_correct_files)
            self.right_content_title.addItems(all_correct_files) #will be filtered properly later
        ...
        for char in INDIV_CHARACTERISTICS:
            ...
            char_values_layout = QHBoxLayout()

            char_label = QLabel(char)
            value_file1_label = QLabel("-")
            value_file2_label = QLabel("-")
            self.file1_result_labels[char] = value_file1_label
            self.file2_result_labels[char] = value_file2_label
            char_values_layout.addWidget(char_label)
            char_values_layout.addWidget(value_file1_label)
            char_values_layout.addWidget(value_file2_label)

            layout.addLayout(char_values_layout)
        ...
        for common_char in self.textual_display_common_charcs:
            ...
            char_values_layout.addWidget(char_label)
            char_values_layout.addWidget(value_file1_label)
            self.common_result_labels[common_char] = value_file1_label
        ...
        if isinstance(self.main_window.final_results, CrossCompare):
    ...
    def _sync_comboboxes(self, changed_combobox):
        newly_selected_item = changed_combobox.currentText()
